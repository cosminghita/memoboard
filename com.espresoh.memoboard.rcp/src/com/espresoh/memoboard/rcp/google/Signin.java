...
public class Signin
{
	public static final String LOGIN_SERVER_URL = "http://localhost:3333";
	 ...
	 */
	private static final String APPLICATION_NAME = "MemoBoard";
	...
	public static Server login() throws Exception, InterruptedException
	{
		if (instance == null)
		{
			instance = new Server(3333);
			ServletHandler servletHandler = new ServletHandler();
			SessionHandler sessionHandler = new SessionHandler();
			sessionHandler.setHandler(servletHandler);
			instance.setHandler(sessionHandler);
			...
			Desktop.getDesktop().browse(new URI(LOGIN_SERVER_URL));
...
//				public void run()
//				{
//					new GoogleSigninDialog(Display.getDefault().getActiveShell()).open();
//				}
//			});
	...
	public static class MainServlet extends HttpServlet
	{
		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
		{
			...
			try
			{
				...
				response.getWriter()
						.print(new Scanner(Signin.class.getResourceAsStream("index.html"), "UTF-8").useDelimiter("\\A").next().replaceAll("[{]{2}\\s*CLIENT_ID\\s*[}]{2}", CLIENT_ID)
								.replaceAll("[{]{2}\\s*STATE\\s*[}]{2}", state).replaceAll("[{]{2}\\s*APPLICATION_NAME\\s*[}]{2}", APPLICATION_NAME).toString());
	...
	/**
	 * Upgrade given auth code to token, and store it in the session. POST body of request should be the authorization code. Example URI:
	...
	public static class ConnectServlet extends HttpServlet
	{
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
		{
			...
			try
			{
				...
				GoogleTokenResponse tokenResponse = new GoogleAuthorizationCodeTokenRequest(TRANSPORT, JSON_FACTORY, CLIENT_ID, CLIENT_SECRET, code,
						"postmessage").execute();
				...
				loginMemoboard(tokenResponse.getAccessToken());
		...
		private void loginMemoboard(String tokenResponse) throws IOException
		{
			// --------------------- Collect user information so we can login -----------------------

			
			GoogleCredential credential = new GoogleCredential().setAccessToken(tokenResponse);
			...
			userPO.setName(mePerson.getDisplayName());
			userPO.setAvatar(mePerson.getImage().getUrl());
			Map<Object, Object> emails = (Map<Object, Object>) ((List<?>)mePerson.getUnknownKeys().get("emails")).get(0);
			userPO.setEmail((String) emails.get("value"));
			Display.getDefault().syncExec(new Runnable() {
				
				@Override
				public void run()
				{
					Session.getInstance().login(userPO);
					
					//Make sure we refresh the view after we logged in:
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
		{
			...
			try
			{
				...
				GoogleCredential credential = new GoogleCredential.Builder().setJsonFactory(JSON_FACTORY).setTransport(TRANSPORT)
						.setClientSecrets(CLIENT_ID, CLIENT_SECRET).build().setFromTokenResponse(JSON_FACTORY.fromString(tokenData, GoogleTokenResponse.class));
