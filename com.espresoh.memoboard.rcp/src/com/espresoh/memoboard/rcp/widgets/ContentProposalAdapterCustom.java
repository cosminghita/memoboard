 ...
 * @since 3.2
 */
public class ContentProposalAdapterCustom {
	//	private final static Logger LOG = Logger.getLogger(ContentProposalAdapterCustom.class);
	// if char is allowed is true
	private boolean charAllowed = true;
	...
	class ContentProposalPopup extends PopupDialog {
		...
		private final class TargetControlListener implements Listener {
			...
			public void handleEvent(final Event e) {
				...
				if (e.type == SWT.Traverse) {
					...
					// LOG.debug("traverse event contains a legitimate
					// character**: " + (key != 0));
					if (key != 0) {
						e.doit = false;
						return;
				...
				if(e.keyCode == 32 && e.stateMask == SWT.CTRL){
					e.doit = false;
					return;
				}
				...
				case SWT.LF:
				case SWT.CR:
					//					LOG.debug("Enter pressed******popoup: " + popup);
					// e.doit = false;
					...
					// LOG.debug("acceptCurrentProposal isEnabled: " +
					// isEnabled);
				...
				case SWT.BS:
					...
					// is space
				case 32:
					if (getControlContentAdapter().getCursorPosition(getControl()) == 0 || isReadonly) {
						e.doit = false;
						return;
					}
				...
				default:
					...
					if (Character.isDefined(key)) {
						if (filterStyle == FILTER_CUMULATIVE) {
							filterText = ((Text) control).getText();
							//							LOG.debug("*****filterText before: " + filterText + "   ((Text)control).getText()" + ((Text) control).getText());
							//							LOG.debug("getControlContentAdapter().getCursorPosition(getControl()): "
							//									+ getControlContentAdapter().getCursorPosition(getControl()));
							if (String.valueOf(key).equals("*")) {
								if (getControlContentAdapter().getCursorPosition(getControl()) == 0)
									filterText = String.valueOf(key) + filterText;
								else {
									charAllowed = false;
									return;
							...
							if (isReadonly) {
								if (filterProposals(getProposals(), filterText).length == 0) {
									filterText = filterText.substring(0, filterText.length() - 1);
									e.doit = false;
								}
							} else {
							...
							//							LOG.debug("*****filterText after: " + filterText + "   ((Text)control).getText()" + ((Text) control).getText());
		...
		private class InfoPopupDialog extends PopupDialog {
			...
			protected void adjustBounds() {
				...
				Rectangle rightProposedBounds = new Rectangle(parentBounds.x + parentBounds.width + PopupDialog.POPUP_HORIZONTALSPACING, parentBounds.y
						+ PopupDialog.POPUP_VERTICALSPACING, parentBounds.width, parentBounds.height);
				rightProposedBounds = getConstrainedShellBounds(rightProposedBounds);
				// If it won't fit on the right, try the left
				if (rightProposedBounds.intersects(parentBounds)) {
					Rectangle leftProposedBounds = new Rectangle(parentBounds.x - parentBounds.width - POPUP_HORIZONTALSPACING - 1, parentBounds.y,
		 ...
		 */
		private void changeDefaultColors(final Control control) {
		...
		protected void adjustBounds() {
			...
			if (popupSize == null) {
				...
				// POPUP_MINIMUM_WIDTH);
				data.widthHint = Math.max(control.getSize().x, control.getSize().y);
		...
		private IContentProposal[] filterProposals(final IContentProposal[] proposals, final String filterString) {
			...
			for (int i = 0; i < proposals.length; i++) {
				...
				if (filterString.startsWith("*")) {
					// filterString without star (*)
					String filStr = filterString.substring(1);
					if (string.length() >= filStr.length() && string.toUpperCase().contains(filStr.toUpperCase())) {
		...
		protected InfoPopupDialog getInfoPopup() {
			return infoPopup;
		}
	}
	 ...
	 *            TODO
	 */
	public ContentProposalAdapterCustom(final Control control, final IControlContentAdapter controlContentAdapter, final IContentProposalProvider proposalProvider,
			final KeyStroke keyStroke, final char[] autoActivationCharacters, final boolean isReadonly) {
		super();
	 ...
	 */
	public void addContentProposalListener(final IContentProposalListener2 listener) {
	...
	private void addControlListener(final Control control) {
		...
		controlListener = new Listener() {
			public void handleEvent(final Event e) {
				...
				switch (e.type) {
				case SWT.Traverse:
					// LOG.debug((e.keyCode == SWT.CR) + " - " + isEnabled +
					// "popup: " + popup);
					// LOG.debug(e.keyCode != SWT.CR && e.keyCode != SWT.LF);
				...
				case SWT.KeyDown:
					...
					// LOG.debug("popup before1: " + popup + " e.keyCode ==
					// SWT.TAB: " + (e.keyCode == SWT.TAB));
					if (popup == null && e.keyCode != SWT.TAB && e.keyCode != SWT.CR && e.keyCode != SWT.LF) {
						// autoActivate();
						openProposalPopup();
						// LOG.debug("Open popup **************");
					}
					// LOG.debug("popup after2: " + popup);
					if (popup != null) {
					...
					// by the popup itself.
				case SWT.Modify:
					if (triggerKeyStroke == null && autoActivateString == null && !modifyingControlContent) {
						if (DEBUG) {
	...
	/**
	 * return selected proposal is exist oterwise return null;
	 * 
	 * @return
	 */
	public IContentProposal getSelectedProposal() {
		if (popup != null)
			return popup.getSelectedProposal();
		return null;
	}
	...
	public void setCharAllowed(final boolean charAllowed) {
